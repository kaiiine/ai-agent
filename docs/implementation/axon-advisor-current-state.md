...
**Sizing COMBO FERMÉ (ADR-ADV-014, Option B).** Le fork money-sensitive est tranché. `allow_combos=False` → builder non appelé (comportement inchangé). `allow_combos=True` → combos admissibles **sizés et matérialisés** en `PortfolioLine` COMBO. Le sizing **réutilise EXACTEMENT le Kelly canonique** (`compute_single_stake` sur un `CandidateBet` synthétique : `combined_prob_low`, `combined_odds`, min qualité/reliability des jambes, `exposure_keys` = **union** des jambes) sous une `SizingProfile` COMBO (`combo_fractional_kelly ≤ single`, invariant validé au chargement). **Aucune 2e formule Kelly.** `safety_margin` appliquée **une seule fois** (pricing), jamais au stake. Conservatisme réparti : pricing prudent + `combo_fractional_kelly` + caps d'exposition. **Allocation** : combos alloués APRÈS les singles, dans le **portefeuille primaire uniquement**, budget/exposition **partagés** — un combo consomme le budget **une fois** et son stake compte sur **chaque jambe** (exposition/concentration) ; les singles sont **inchangés** (alloués avant). Invariant zéro-stake préservé (Kelly ≤ 0 / bankroll 0 / cap 0 / arrondi 0 → aucune ligne COMBO). **Audit** : `ComboMaterializationStatus.MATERIALIZED` (ou `ADMISSIBLE_NOT_MATERIALIZED` si écarté par les caps) ; `BLOCKED_SIZING_NOT_AVAILABLE` conservé pour la désérialisation d'audits historiques, jamais produit par le code courant ; `REPLAY_EXACT` préservé (round-trip avec ligne COMBO financée testé). **Travaux différés** : combos 3 legs, moteur de règles sportives, estimation statistique de proba jointe, optimisation globale singles+combos.

### 10.7 Décisions tranchées — Lot 10 (Audit persistant & replay exact)

ADR dédiés : `ADR-ADV-012-audit-persistence.md`, `ADR-ADV-013-audit-envelope-replay.md`.

**§0 — unicité de `request_id` (prérequis bloquant).** Vérification sur le code réel : le CLI fixait `--request-id` à `"axon-cli"` (NON unique). **Tranché dans ce lot** : le CLI génère un `request_id` FRAIS par invocation (`req:<uuid4>`) quand aucun n'est fourni ; un `--request-id` explicite reste possible pour un rejeu ciblé. L'idempotence par `request_fingerprint` repose sur cette unicité.

**Backend & emplacement.** Store **JSONL append-only** (une enveloppe canonique par ligne), encapsulé derrière `JsonlAuditStore` (un autre backend pourra le remplacer sans toucher au domaine). **SQLite et base Axon rejetés en V1** (surdimensionné / couplant). Chemin **injectable** (tests : `tmp_path`) ; défaut repo-local `var/advisor/audits/audit.jsonl` (`var/` gitignoré), en config versionnée `configs/advisor/audit_policy.json`. **`~/.axon` interdit** (interdiction stable, validée par le loader).

**Séparation stricte.** `RecommendationResponse` reste **gelée** (aucun champ ajouté, aucun rendu optionnel). L'audit est un artefact **parallèle** : `run_pipeline` renvoie `PipelineRunResult{recommendation, trace}` ; l'enveloppe est construite à partir de la trace, jamais dans la réponse.

**Sérialisation canonique unique.** Réutilise la primitive du Lot 1 (`domain.serialization.to_jsonable/to_json`) — étendue une fois pour gérer les `float` **diagnostiques non monétaires** (valeurs de features → chaîne canonique) ; la sécurité monétaire reste garantie par les **contrats** (`Decimal` obligatoire), pas par le rejet du sérialiseur. Clés triées, `Decimal`→str, `Enum`→valeur, `Mapping` déterministe, indépendance à l'ordre d'insertion.

**Enveloppe versionnée + checksum.** `AdvisorAuditEnvelope{audit_schema_version, audit_id, request_id, request_fingerprint, created_at, payload_checksum, payload}`. `payload_checksum = sha256(canonical(payload))` (intégrité de l'archive, PAS un pointeur vers l'état disque courant). `created_at` = métadonnée : hors identité et hors checksum du payload (n'affecte jamais l'idempotence). Version inconnue à la lecture → erreur explicite, jamais de migration silencieuse.

**Snapshots complets des configs.** Le payload archive le **contenu exact** des configs consommées (`ConfigSnapshot{config_name, config_version, checksum, content}`) — combo uniquement si `allow_combos`. Le replay reconstruit les configs depuis ces snapshots (via fichier temporaire + loader réel), **jamais** depuis `configs/advisor/*.json` courant : une config modifiée/supprimée six mois plus tard ne casse pas un replay historique. Snapshot corrompu (checksum≠contenu) → `CONFIG_SNAPSHOT_CORRUPT` avant exécution.

**Identité & idempotence.** `request_fingerprint = sha256(canonical(requête SANS request_id))` (contenu métier, hors `created_at`/ordre/chemin). `audit_id = "audit:"+sha256(request_id | fingerprint)`. Même `request_id`+contenu → même `audit_id` → append **idempotent** (no-op). Même `request_id`+contenu différent → **`REQUEST_ID_CONTENT_MISMATCH`**. Même `audit_id`+payload divergent → **`DUPLICATE_AUDIT_DIVERGENT`**. Autre `request_id`+même contenu → `audit_id` différent (deux exécutions attendues). **Aucun overwrite.**

**Replay `REPLAY_EXACT` (seul mode V1).** `load → vérifie version → vérifie payload_checksum → vérifie snapshots → reconstruit requête/configs/batch archivés → rejoue le pipeline déterministe → compare le résultat MÉTIER (recommandation + trail Combo) au résultat archivé`. Contrat de sortie **structuré** `ReplayResult{matches, differences}` (exploitable hors tests). **Aucun** appel bookmaker/Betting Engine/Gateway ; aucune dépendance aux configs courantes. « Identique » = métier déterministe (candidate_id, statuts, reason codes, scores, stakes, allocations, combo ids/pricing/statuts, réponse), **hors** métadonnées runtime (`created_at`, durée, chemin). **Hors scope** : `COMPARE_CURRENT`/drift, migrations de schéma, replay cross-version de code incompatible (le code lisant ce schéma doit exister ; V1 snapshotte données+configs, pas l'exécutable Python).

**Quatre états Combo archivés EXPLICITEMENT** (jamais déduits des `PortfolioLine`) : `ComboMaterializationStatus` ∈ {`NOT_APPLICABLE` (allow_combos=False), `NO_CANDIDATE` (invoqué, aucun admissible), `MATERIALIZED` (jamais V1), `BLOCKED_SIZING_NOT_AVAILABLE`} + `ComboBookmakerAcceptanceStatus` ∈ {`NOT_VERIFIED` (V1, aucune donnée), `ACCEPTED`, `REJECTED`} + `combo_builder_invoked` + `combo_signal`. **Le sizing COMBO n'est PAS résolu.**

**Robustesse & limites.** Reader strict (JSON invalide → `INVALID_AUDIT_JSON`, champ absent → `AUDIT_INCOMPLETE`, checksum → `AUDIT_CHECKSUM_MISMATCH`, doublon contradictoire → `DUPLICATE_AUDIT_DIVERGENT`, introuvable → `AUDIT_NOT_FOUND`) — jamais de réparation silencieuse. Écriture = ligne canonique complète en append. **Concurrence multi-processus hors scope V1** (JSONL append-only mono-processus ; un vrai backend transactionnel pourra venir derrière `JsonlAuditStore`). Secrets d'infrastructure jamais archivés (seules les données métier de replay le sont).
