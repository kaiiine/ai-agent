...
Ce module sépare donc deux questions que la liste confondait :

    « est-ce une écriture ? »      → détection structurelle, pas lexicale
    « est-elle locale ou distante ? » → décide du traitement

Le local reste refusé : `edit_file` fait mieux, avec un diff relu. Le distant
n'a pas d'équivalent, il passe donc par une confirmation — mais une
confirmation qui montre le CONTENU à écrire, pas seulement la commande.
"""
...
def _deshabiller(fragment: str) -> str:
    """Retire les guillemets qui enveloppent la commande distante."""
    fragment = fragment.strip()
    for q in ('"', "'"):
        if len(fragment) >= 2 and fragment.startswith(q) and fragment.endswith(q):
            return fragment[1:-1]
    return fragment


def _contenu_de(commande: str) -> str | None:
    """Le texte qui sera écrit, ou None s'il ne se lit pas dans la commande."""
...
def analyser_ecriture(commande: str) -> Ecriture | None:
    ...
    if m:
        return Ecriture(cible=m.group(3), ajoute=False, outil=m.group(1),
                        distante=True, hote=m.group(2), contenu=None)
    ...
    if redirection:
        ...
        return Ecriture(cible=cible, ajoute=redirection.group(1) == ">>",
                        ...
                        hote=hote, contenu=_contenu_de(interne))
    return None
